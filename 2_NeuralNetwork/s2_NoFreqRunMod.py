...
def build_network(modinput): # (input_noiserate, weight_noiseinput):
    ...
    # # Dynamics
    active_spiketimes=np.arange(300.,700.,5.)   # Active input
    ...
    nest.ConvergentConnect(pop_ca3i, sd_ca3i, model='readout')
    ...
    nest.ConvergentConnect(pop_ca1i, sd_ca1i, model='readout')
    ...
    # Noise to nodes
    nest.DivergentConnect(input_noise, pop_ca3i, weight_noiseinput*0.1, dt)  # These give 4ish for CA3/1, but 2is for Amydala
    nest.DivergentConnect(input_noise, pop_amy, weight_noiseinput*0.95, dt)  
    nest.DivergentConnect(input_noise, pop_ca1e, weight_noiseinput*0.741, dt) 
...
def dochartsmeans(mo):
    pop1=mo['ca3e_raster'].mean(axis=1)
    pop2= mo['ca1e_raster'].mean(axis=1)
    pop3= mo['amy_raster'].mean(axis=1)
    two_sample = spstats.ttest_ind(pop1, pop2)
    # print '[ca3 vs ca1] T=' + str(two_sample[0])  + ' , p=' +  str(two_sample[1]) 
    ind = np.array([0, 1., 2.])  # the x locations for the groups
    width = 0.35       # the width of the bars
    popmeans= [pop1.mean()*1000,  pop2.mean()*1000,  pop3.mean()*1000]
    popses= [np.std(pop1)*1000/np.sqrt(pop1.shape[0]) , np.std(pop2)*1000/np.sqrt(pop2.shape[0]), np.std(pop3)*1000/np.sqrt(pop3.shape[0])] 
    plt.bar(ind[0]+width, popmeans[0], width, color='y', yerr=popses[0]*2)
    plt.bar(ind[1]+width, popmeans[1], width, color='b', yerr=popses[1]*2)
    plt.bar(ind[2]+width, popmeans[2], width, color='g', yerr=popses[2]*2)
    # plt.xlim(xmax=1.4, xmin=0.2)    
    plt.ylim(ymax=5., ymin=1.5)    
    plt.title('Mean firing rate (CA3 vs CA1 vs Amygdala)')
    # plt.set_xticks([0.5, 1.05])
    # plt.set_xticklabels(['CA3', 'CA1'])
    plt.show()
    print popmeans
    print popses


""" DOING PLOTS JUST FOR STUFF I WANT """

def JustPlotResults2Show(mo):
    ...
    plt.xlim(xmax=mo['time_ms'], xmin=0)
    plt.xticks(range(0, int(mo['time_ms'])+1, fig_xtick_ms))
    # plt.yticks(range(0,   fig_plotspikes_nneurons+1, fig_ytick_neurons))
    k=k+1
    ...
    try:
        ...
        plt.xticks(range(0, 1000., fig_xtick_ms))
        # plt.yticks(range(0,   fig_plotspikes_nneurons+1, fig_ytick_neurons))
        k=k+1
    except:
        pass
    ...
    # CUTOUTS ####################################################
    cutout_xminmax=[1500, 2500]
    cutout_xtick=(cutout_xminmax[1] - cutout_xminmax[0])/2
    ...
    plt.scatter(spiketimes, neuronidlist, s=fig_rasterdot_size)
    plt.yticks(range(0,   fig_plotspikes_nneurons+1, fig_ytick_neurons))
    plt.xlim(xmax=cutout_xminmax[1], xmin=cutout_xminmax[0])
    plt.ylim(ymin= np.unique(neuronidlist)[0],  ymax=np.unique(neuronidlist)[0]+fig_plotspikes_nneurons)
    plt.xticks(range(cutout_xminmax[0], cutout_xminmax[1]+1,  cutout_xtick))
    ...
    # Display !! 
    plt.subplots_adjust(left=0.05, bottom=None, right=0.99, top=None, wspace=0.7, hspace=0.5)
    # savefig('2_Simulated_data\Plots4Results.png')
    plt.savefig('Plots4Results.png')
    print 'Move figure + spikedata if you want to save this simulation!!!!!'
    plt.show()
