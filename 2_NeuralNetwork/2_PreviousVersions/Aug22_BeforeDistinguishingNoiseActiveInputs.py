...
def build_network(modinput): # (input_noiserate, weight_input2ca3):
    ...
    # input_noiserate=19800.    # input rate & weight = 19800 & 9 balances CA3 at a higher rate
    # weight_input2ca3=9.
    input_noiserate=20500.      # input rate & weight= 20500 & 8.25 will get CA3 down to <10 Hz, but excit and inhib is oscillating
    weight_input2ca3=8.25
    ...
    c_ca3_2amye=[0.4,  3.*c_fudgeweights]
    ...
    nest.Connect(input_noise, sd_input, model='readout')
    ...
    # c_ca3_2ca1i= [0.1825,  0.4*c_fudgeweights]  # CA3 --> CA1 interneurons
    # # Weights from here are arbitrary !!! 
    # c_ca3_2amye=[0.5,  0.8*c_fudgeweights]
    # c_amye_2ca1e=[0.3,  1.5*c_fudgeweights]



    """ Connectivity """
    nest.DivergentConnect(input_noise, pope_ca3, weight_input2ca3, dt)  
    # Within CA3
    ...
    # nest.DivergentConnect(input_noise, popi_ca3, weight_input2ca3*0.05, dt)  
...
def scan_weights():
    ...
    # Settings 
    input_noiserates =np.arange(18000.,22000., 250.)
    input_weights =  np.arange(8., 11., 0.25)   
    ...
    for s0 in range(len(scanpar[0])):
        for s1 in range(len(scanpar[1])):
            ...
            print 'Run # ', k
            modinput={'input_noiserate':  input_noiserates[s0], 'weight_input2ca3': input_weights[s1]}
...
# nest.GetStatus(mo['sd_ca3'],  'n_events')
